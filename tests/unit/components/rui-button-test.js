...
test('it displays block class when block property is set', function(assert) {
  ...
  assert.notOk(this.$().hasClass('btn-block'), 'it should not have \'btn-block\' by default');

  Ember.run(function(){
    component.set('block', true);
  });
  assert.ok(this.$().hasClass('btn-block'), 'it should have \'btn-block\' class when block is set to \'true\'');
});
...
test('it disables when disable property is set to true', function(assert) {
  ...
  assert.notEqual(this.$().attr('disabled'), 'disabled', 'it should not be disabled by default');

  Ember.run(function(){
    component.set('disabled', true);
  });

  assert.equal(this.$().attr('disabled'), 'disabled', 'it should be disabled when disabled property is set to \'true\'');
});
